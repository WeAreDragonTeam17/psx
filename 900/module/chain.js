...
export class ChainBase {
    ...
    check_allow_run() {
        ...
        if (this.is_dirty) {
            throw Error('Đã chạy chuỗi này rồi, hãy làm sạch nó trước');
        }
    }
    ...
    push_value(value) {
        ...
        if (position >= this.stack_size) {
            throw Error(`Hết dung lượng trên stack, đã đẩy giá trị: ${value}`);
        }
    ...
    get_gadget(insn_str) {
        ...
        if (addr === undefined) {
            throw Error(`Không tìm thấy tiện ích: ${insn_str}`);
        }
    ...
    push_syscall(syscall_name, ...args) {
        if (typeof syscall_name !== 'string') {
            throw TypeError(`syscall_name không phải chuỗi: ${syscall_name}`);
        }

        const sysno = syscall_map.get(syscall_name);
        if (sysno === undefined) {
            throw Error(`syscall_name không tìm thấy: ${syscall_name}`);
        }

        const syscall_addr = this.syscall_array[sysno];
        if (syscall_addr === undefined) {
            throw Error(`số lệnh hệ thống không có trong syscall_array: ${sysno}`);
        }
    ...
    push_end() {
        throw Error('chưa được triển khai');
    }
...
function load_fw_specific(version) {
    ...
    if (value < 0x900) {
        throw RangeError("Các phiên bản phần mềm hệ thống PS4 nhỏ hơn 9.00 không được hỗ trợ");
    }
    ...
    throw RangeError('phiên bản phần mềm hệ thống không được hỗ trợ');
}
